...
,'',original.stem)
    if original.exists():variants[99999]=original
    for candidate in original.parent.glob(base_stem+'-*x*'+original.suffix):
        match=re.search(r'-(\\d+)x(\\d+)
env.globals['image_srcset']=image_srcset
def asset_link(path):
    fingerprint=hashlib.sha256((OUT/path.lstrip('/')).read_bytes()).hexdigest()[:12]
    return link(path)+'?v='+fingerprint
...
,candidate.stem)
        ...
        width=int(match.group(1))
        if 240<=width<=1600:variants[width]=candidate
    return ', '.join(f"{link('/'+str(candidate.relative_to(OUT)))} {width}w" for width,candidate in sorted(variants.items()) if width!=99999)
env.globals['image_srcset']=image_srcset
def asset_link(path):
    fingerprint=hashlib.sha256((OUT/path.lstrip('/')).read_bytes()).hexdigest()[:12]
    return link(path)+'?v='+fingerprint
